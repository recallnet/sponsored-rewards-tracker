...
import {
  ...
  formatRelative, formatTimeLeft, addr, polymarketUrl,
} from './shared';
...
export default function SponsoredTab() {
  ...
  if (error || !data)
    return <ErrorBlock message={error instanceof Error ? error.message : undefined} label="sponsored rewards" />;
      ...
      <div className="overflow-x-auto border border-[#333]">
              ...
              return (
                <Fragment key={r.txHash}>
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-right font-mono">{formatCurrency(r.amountUsdc)}</td>
                    <td className="py-3 px-4 text-right font-mono">{formatCurrency(r.ratePerDayUsdc)}/d</td>
                      ...
                      <td colSpan={5} className="px-5 py-4 text-[#999]">
                        <div className="space-y-1 text-xs font-mono">
                          <p>
                            ...
                            {r.consumedUsdc > 0 && ` — consumed ${formatCurrency(r.consumedUsdc)} (${pct}%)`}
                          </p>
                          {r.returnedUsdc > 0 && <p>Returned {formatCurrency(r.returnedUsdc)}</p>}
                          <p>
                            {new Date(r.startTime).toLocaleDateString()} →{' '}
                            {new Date(r.endTime).toLocaleDateString()} ({r.durationDays}d total, {timeLeft} remaining)
                          </p>
                          <p>
                            tx{' '}
                            <a href={`https://polygonscan.com/tx/${r.txHash}`} target="_blank" rel="noopener noreferrer" className="hover:underline" onClick={e => e.stopPropagation()}>
                              {r.txHash.slice(0, 22)}…
                            </a>
                          </p>
                        </div>
        ...
        {hasMore && (
          <div className="border-t border-[#333] px-5 py-3 flex items-center justify-between">
            <p className="text-xs text-[#555] font-mono">Showing {visible.length} of {active.length}</p>
      ...
      {topSponsors.length > 0 && (
        <div className="mt-14">
          ...
          <div className="border border-[#333] divide-y divide-[#333]">
            {topSponsors.map(s => (
              <div key={s.address} className="flex items-center justify-between px-5 py-3 hover:bg-[#0a0a0a] transition-colors">
                <a href={`https://polymarket.com/profile/${s.address}`} target="_blank" rel="noopener noreferrer" className="font-mono text-sm hover:underline">
                  {addr(s.address)}
                </a>
                <div className="flex items-center gap-8 text-sm">
                  <span className="font-mono">{formatCurrency(s.net)}</span>
                  <span className="text-[#555] tabular-nums">{s.count} market{s.count !== 1 ? 's' : ''}</span>
                </div>
              </div>
            ))}
          </div>
